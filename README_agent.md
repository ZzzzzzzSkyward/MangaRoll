...
- **拖拽导入**：将文件夹或 ZIP 压缩包拖入页面即可打开其中的图片，无需上传
  ...
  - ZIP 使用 [unzipit](https://github.com/greggman/unzipit) 在浏览器内解压（按需读取：先读文件尾部中央目录，仅解压所需条目）
  ...
  - **文件夹列表视图**：打开的文件夹具有层级结构（含子文件夹）时自动进入列表视图（占据漫画阅读区域），按自然顺序列举所有子文件夹；封面取子文件夹内第一张图片，若存在主名完全等于 `cover`（不含扩展名、不区分大小写）的图片则优先作为封面。目录同时包含图片与子文件夹时，图片视为本层漫画（列表首项带「本目录」标记），点击即读。点击条目：含子文件夹则下钻一层，仅含图片则直接打开为漫画（进度按目录独立记忆，目录内弹幕 JSON 自动加载）；列表内可逐级返回，阅读中可通过工具栏「返回目录」回到列表。「项目数」列只统计该层直接包含的图片数、置于子文件夹 / 压缩包内的不递归计入（压缩包条目统计包内全部图片，即本压缩文件内部）
  ...
  - **ZIP 封面提取与条目数统计**：列表视图中的压缩包条目自动提取封面、列表「项目数」列显示包内图片数量（本压缩文件内部全部图片，不递归压缩包内部子目录）——均为分块读取压缩包（先读文件尾部中央目录定位条目，不整体加载解压），封面选择逻辑与文件夹一致（优先主名完全等于 `cover`、不区分大小写的图片，否则取第一张），仅解压封面单条目；结果缓存到内存（会话内不重复读取，切换列表只释放临时对象 URL），条目数可供「项目数」列与按项目数排序使用
- **图片格式支持**
...
└── src/
    ...
    ├── style.css            # 全局样式与主题变量
    ├── lib/
    │   ├── importer.js      # 文件夹递归遍历、ZIP 解压（unzipit）、ZIP 封面提取 / 条目数统计、图片尺寸提取、最大尺寸缩小、自然排序、文件夹树构建
...
| ZIP 解压 | unzipit（npm 依赖）：读文件尾部中央目录 + 按条目随机访问解压，不整体加载 / 解压；封面提取（`extractZipCover`）与条目数统计（`countZipImages`）仅读目录 / 解压封面单条目 |
...
- [x] ZIP 封面提取与条目数统计（分块读目录 + 单条目解压 + 内存缓存）
- [ ] 横向模式下按页预取 / 翻页式单页模式
- [ ] 支持 `.rar` / `.cbz` 等更多格式
- [ ] 弹幕字幕（.ass / .srt）导入
